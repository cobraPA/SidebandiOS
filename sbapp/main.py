...
import os
## kjb cep - merged out?
### no - conditional import based on daemon - revisit
#import plyer
import pathlib
import base64
import threading
...
if RNS.vendor.platformutils.get_platform() == "ios":
    #import threading_ios as threading
    # left, bottom, right, top - dict
    
    # leads to a threading RLock error, because ios imports webbrowser
    # disable for testing
    from ios import get_safe_area
...
if not args.daemon:
    ...
    if RNS.vendor.platformutils.get_platform() != "android":
        ...
        #kjb cep
        print(" file path: ", __file__, " dirname: ", local)
...
if args.daemon:
    ...
    if RNS.vendor.platformutils.get_platform() == "android":
        ...
        # kjb fix - disabled this on kivy:master - fixes the issue with preferences and keyboard popup
...
# kjb CEP
# kivy documents this, but kivyMD requires it to be in build()
#Builder.load_string(root_layout)
...
class SidebandApp(MDApp):
    ...
    # kjb threading
    def queue_main_thread_loop(self, dt):
        next_task = self.queue_main_thread_tasks.get()
        if next_task != None:
            next_task()
    ...
    def __init__(self, **kwargs):
        ...
        if RNS.vendor.platformutils.get_platform() == "ios":
            # kjb threading
            self.queue_main_thread_tasks = Queue()
            #Clock.schedule_interval(self.queue_main_thread_loop, .2)
        else:
            self.queue_main_thread_tasks = None

        if RNS.vendor.platformutils.get_platform() == "android":
            self.sideband = SidebandCore(self, is_client=True, android_app_dir=self.app_dir, verbose=__debug_build__)
        elif RNS.vendor.platformutils.get_platform() == "ios":
#            self.sideband = SidebandCore(self, is_client=False, verbose=(args.verbose or __debug_build__), main_queue=self.queue_main_thread_tasks)
	...
	# kjb cep - old version of block above
        #self.sideband.setstate("app.loaded", True)
        #self.sideband.setstate("app.running", True)
        #self.sideband.setstate("app.foreground", True)
    ...
    def start_service(self):
        ...
        # kjb info
            ...
            RNS.log("Launching platform-specific service: nothing done, Android only")
    ...
    def set_ui_theme(self):
        ...
        # no obvious change when ios applied to widgets...
        if RNS.vendor.platformutils.get_platform() == "ios":
            self.theme_cls.widget_style = "ios"           
        else:
    ...
    def build(self):
        ...
        print('screen- ', screen.__dict__.keys())
        
        self.nav_drawer = screen.ids.nav_drawer
        
        ## test 4
        #aa = Builder.load_string(root_layout)
        #print('aa- ', aa.__dict__.keys())
        #sm = ScreenManager()
        #print('screen- ', sm.__dict__.keys())
        # kjb test - TODO
        #self.sm = screen._screen_manager
        # test 2
        #self.sm = sm
        ##self.app.root.ids.screen_manager = screen._screen_manager

        ##print( str(screen._screen_manager.size) + " top "  + str(screen._screen_manager.top) + " h " + str(screen._screen_manager.height) )
        
        # kjb todo - use get_safe_area()
        #if RNS.vendor.platformutils.get_platform() == "ios":
            ##safe_ios = get_safe_area()
            ##print( "safe ... ", safe_ios)
            #screen.root.top = safe_ios["top"]
            # no error, screen doesn't change
            #screen._screen_manager.size[0] = 90
            # Works to shrink screen down, buttons visible
            #screen._screen_manager.size_hint_y = 0.95
            ##screen._screen_manager.size_hint_y = 0.93
            #screen._screen_manager.size_hint_y = None
            # no self.height, or max_height
            #screen._screen_manager.max_height = self.max_height - safe_ios["top"]
            # https://github.com/kivy/kivy-ios/issues/189
            
            #print(' pos_hint '+str(screen.pos_hint))
            #screen.pos_hint={'top': 0.8}
            # same as screen._screen_manager...
            #screen.size_hint_y = 0.8
	...
	# kjb - disabled for iOS
        if RNS.vendor.platformutils.get_platform() != "ios":
            EventLoop.window.bind(on_keyboard=self.keyboard_event)
            EventLoop.window.bind(on_key_down=self.keydown_event)

        # kjb
        # setup notch offset for handheld screen
        if RNS.vendor.platformutils.get_platform() == "ios":
            #print( ' nav ' +str(self.root.ids.nav_drawer.pos_hint) + ' radius ' + str(self.root.ids.nav_drawer.radius)) 
            #print(' dp ' + str(dp(1)) + ' ' +str(dp(10)))
            # nav {} radius [0, 24.0, 24.0, 0], dp =3
            # sets nav_drawer 80% of screen
            #self.root.ids.nav_drawer.pos_hint ={'top': 0.8}

            safe_ios = get_safe_area()
            #print( "safe ... ", safe_ios)
            # safe ...  {'top': 59.0, 'bottom': 34.0, 'right': 0.0, 'left': 0.0}
            #height = dp(1) * 100
            #top = height - safe_ios["top"]
            #new_scale = height / top
            #print('calcs ' + str(height) + ' safe top '+ str(top) + ' scale ' + str(new_scale))
            new_scale = (self.root.height - (safe_ios['top'] * Metrics.dp) ) / self.root.height
            self.root.ids.nav_drawer.pos_hint ={'top': new_scale}
            #print("new scale " + str(new_scale))
            #self.screen._screen_manager.size_hint_y = 0.93
            self.root.ids.screen_manager.size_hint_y = new_scale

            
            #print(' dp ' + str(Metrics.dp) + ' density ' + str(Metrics.density))
            #print( ' sp ' + str(Metrics.sp))
            #print( ' w ' + str(self.root.width ) + ' h ' + str(self.root.height ))
            # w 1179 h 2556 - iphone
            #screen.root.top = safe_ios["top"]

        # kjb - not needed?  Delete - back key handling fixed in
        # kivy master?  (Or possibly Android only for exit behavior?)
        ##EventLoop.window.bind(on_keyboard=self.keyboard_event)
        # this either?  No ctrl on ios...
        ##EventLoop.window.bind(on_key_down=self.keydown_event)
...
#        elif RNS.vendor.platformutils.get_platform() == "ios":
            # kjb todo
#            connectivity_status = 'iOS status goes here!'
            else:
    ...
    def connectivity_init(self, sender=None):
        if not self.connectivity_ready:
            ...
            if RNS.vendor.platformutils.get_platform() == "android" or RNS.vendor.platformutils.get_platform() == "ios":
                if (not self.sideband.getpersistent("service.is_controlling_connectivity") and RNS.vendor.platformutils.get_platform() == "android"):
            # upstream version
            #if RNS.vendor.platformutils.get_platform() == "android":
                #if not self.sideband.getpersistent("service.is_controlling_connectivity"):
                    RNS.log(' DDDD  - ! service.is_controlling_connectivity ')
...
#                self.sideband.save_configuration()
    ...
    def init_announces_view(self, sender=None):
        if not self.announces_view:
            print('xxx init announces 1')
            self.announces_view = Announces(self)
            print('announces object -', self.announces_view, ' app object- ', self.app)
            self.sideband.setstate("app.flags.new_announces", True)

            print('tree 1', self.root.ids)
  ##          print('tree 2', self.root.ids.announces_scrollview)
    #        print('tree 3', self.root.ids.announces_scrollview.children)
 #           for child in self.root.ids.announces_scrollview.children:
   #             print('xxx init announces 2')
...
def run():
    if args.daemon:
        ...
        # kjb cep - only this line in old version
        SidebandApp().run()
